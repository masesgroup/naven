...
namespace MASES.NavenCLI
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                NavenCLICore.CreateGlobalInstance();

                if (NavenCLICore.MainClassToRun != null)
                {
                    try
                    {
                        NavenCLICore.Launch(NavenCLICore.MainClassToRun, NavenCLICore.FilteredArgs);
                    }
                ...
                else if (!string.IsNullOrEmpty(NavenCLICore.Script))
                {
                    ShowLogo("Script mode");

                    if (!File.Exists(NavenCLICore.Script)) throw new FileNotFoundException("A valid file must be provided", NavenCLICore.Script);

                    var scriptCode = File.ReadAllText(NavenCLICore.Script);

                    ScriptOptions options = ScriptOptions.Default.WithReferences(typeof(JNetCoreBase<>).Assembly, typeof(NavenCLICore<>).Assembly)
                                                                 .WithImports(NavenCLICore.NamespaceList);
        ...
        {
            if (!NavenCLICore.NoLogo)
            {
                Console.WriteLine($"NavenCLI - CLI interface for Naven - Version {_assembly.GetName().Version} - {logoTrailer}");
            }
        }
        ...
        static void ShowHelp(string errorString = null)
        {
            ...
            IDictionary<string, Type> implementedClasses = NavenCLICore.GetMainClasses(typeof(NavenCLICore<>).Assembly);
