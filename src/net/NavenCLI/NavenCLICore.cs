...
namespace MASES.NavenCLI
{
        ...
        where T : NavenCLICore<T>
    {
            ...
            get
            {
                ...
                lst.AddRange(new IArgumentMetadata[]
                {
                    ...
                    new ArgumentMetadata<string>()
                    {
                        ...
                        Type = ArgumentType.Double,
                        Help = "A CSV list of namespace to be used for interactive shell, JNet/Naven namespaces are added automatically",
                    },
                    new ArgumentMetadata<string>()
                    {
        ...
        protected override string[] ProcessCommandLine()
        {
            ...
            var navenAssembly = typeof(NavenCore<>).Assembly;
            foreach (var item in navenAssembly.GetExportedTypes())
            {
                if (item.IsPublic)
                {
                    if (!namespaceList.Contains(item.Namespace)) namespaceList.Add(item.Namespace);
